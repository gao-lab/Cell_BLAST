...
class Regularizer(nn.Module):
    def __init__(
            self, latent_dim: int, h_dim: int = 128, depth: int = 1, dropout: float = 0.0,
            name: str = 'Reg', _class: str = 'Regularizer',
            **kwargs
    ) -> None:
        super().__init__()
...
class Latent(nn.Module):
    ...
    """
    def __init__(
            self, input_dim: int, latent_dim: int, h_dim: int = 128, depth: int = 1,
    ...
    @staticmethod
    def gan_d_loss(y: torch.Tensor, y_hat: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
...
class Gau(Latent):
    ...
    def __init__(self,
        ...
        super().__init__(
            input_dim, latent_dim, h_dim, depth, dropout,
            lambda_reg, fine_tune, deviation_reg, name, _class, **kwargs
        )
    ...
    def d_loss(self, gau: torch.Tensor, feed_dict: typing.Mapping, loss_record: typing.Mapping) -> typing.Tuple[torch.Tensor]:

        gaup = self.gaup_sampler.sample((gau.shape[0], self.latent_dim)).to(config.DEVICE)
        gau_pred = self.gau_reg(gau)
        gaup_pred = self.gau_reg(gaup)
        gau_d_loss = self.gan_d_loss(gau_pred, gaup_pred)
        loss_record[self.record_prefix + '/' + self.name + '/' + self.gau_reg.name + '/d_loss/d_loss'] += gau_d_loss.item() * gau.shape[0]

        return self.lambda_reg * gau_d_loss

    def g_loss(self, gau: torch.Tensor, feed_dict: typing.Mapping, loss_record: typing.Mapping) -> typing.Tuple[torch.Tensor]:

        gau_pred = self.gau_reg(gau)
        gau_g_loss = self.gan_g_loss(gau_pred)
        loss_record[self.record_prefix + '/' + self.name + '/' + self.gau_reg.name + '/g_loss/g_loss'] += gau_g_loss.item() * gau.shape[0]

        if self.fine_tune:
            return self.lambda_reg * gau_g_loss + self.deviation_reg * self.deviation_loss()
        else:
            return self.lambda_reg * gau_g_loss

    def init_loss_record(self, loss_record: typing.Mapping) -> None:

        loss_record[self.record_prefix + '/' + self.name + '/' + self.gau_reg.name + '/d_loss/d_loss'] = 0
        loss_record[self.record_prefix + '/' + self.name + '/' + self.gau_reg.name + '/g_loss/g_loss'] = 0
...
class CatGau(Latent):
    ...
    def deviation_loss(self) -> torch.Tensor:
        return self.deviation_reg * \
            (self.mlp.deviation_loss() + self.gau.deviation_loss() + \
            self.cat.deviation_loss() + self.mat.deviation_loss())
    ...
    def d_loss(self, catgau: typing.Tuple[torch.Tensor], feed_dict: typing.Mapping, loss_record: typing.Mapping) -> torch.Tensor:
        ...
        loss_record[self.record_prefix + '/' + self.name + '/' + self.cat_reg.name + '/d_loss/d_loss'] += cat_d_loss.item() * cat.shape[0]

        return self.lambda_reg * (gau_d_loss + cat_d_loss)

    def g_loss(self, catgau: typing.Tuple[torch.Tensor], feed_dict: typing.Mapping, loss_record: typing.Mapping) -> typing.Tuple[torch.Tensor]:
        ...
        loss_record[self.record_prefix + '/' + self.name + '/' + self.cat_reg.name + '/g_loss/g_loss'] += cat_g_loss.item() * cat.shape[0]

        if self.fine_tune:
            return self.lambda_reg * (gau_g_loss + cat_g_loss) + self.deviation_reg * self.deviation_loss()
        else:
    ...
    def init_loss_record(self, loss_record: typing.Mapping) -> None:
        ...
        loss_record[self.record_prefix + '/' + self.name + '/' + self.gau_reg.name + '/d_loss/d_loss'] = 0
        loss_record[self.record_prefix + '/' + self.name + '/' + self.gau_reg.name + '/g_loss/g_loss'] = 0
        loss_record[self.record_prefix + '/' + self.name + '/' + self.cat_reg.name + '/d_loss/d_loss'] = 0
        loss_record[self.record_prefix + '/' + self.name + '/' + self.cat_reg.name + '/g_loss/g_loss'] = 0

    def parameters_reg(self):
        return itertools.chain(
            self.gau_reg.parameters(),
            self.cat_reg.parameters()
        )
...
class SemiSupervisedCatGau(CatGau):
    ...
    def __init__(
        ...
        super().__init__(
            input_dim, latent_dim, cat_dim, h_dim, depth, dropout,
            lambda_reg, fine_tune, deviation_reg, name, _class, **kwargs
        )
    ...
    def d_loss(self, catgau: typing.Tuple[torch.Tensor], feed_dict: typing.Mapping, loss_record: typing.Mapping) -> torch.Tensor:
        ...
        cat_prob = torch.ones(self.cat_dim) * self.background_catp + cats.cpu().sum(dim = 0)
    ...
    def g_loss(self, catgau: typing.Tuple[torch.Tensor], feed_dict: typing.Mapping, loss_record: typing.Mapping) -> typing.Tuple[torch.Tensor]:
        ...
        loss_record['semi_supervision/' + self.name + '/supervised_loss'] += sup_loss.item() * cats.shape[0]
        ...
        if self.fine_tune:
            return self.lambda_sup * sup_loss + \
                self.lambda_reg * (gau_g_loss + cat_g_loss) + \
                self.deviation_reg * self.deviation_loss()
        else:
            return self.lambda_sup * sup_loss + self.lambda_reg * (gau_g_loss + cat_g_loss)
