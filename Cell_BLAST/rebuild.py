...
class Linear(nn.Module):
    ...
    def __init__(self, in_features: typing.Union[typing.Tuple[int], int], out_features: int, bias: bool = True,
                        init_std: float = 0.01, trunc: bool = True) -> "Linear":
    ...
    def reset_parameters(self):
        ...
        if self.trunc:
            for _weight in self.weight:
                nn.init.trunc_normal_(_weight, std = self.init_std, a = -2 * self.init_std, b = 2 * self.init_std)
        else:
            for _weight in self.weight:
    ...
    def forward(self, input: typing.Union[typing.Tuple[torch.Tensor], torch.Tensor]) -> torch.Tensor:
    ...
    def save_origin_state(self) -> None:
        self.weight_origin = [_weight.detach().clone() \
            for _weight in self.weight]
        if self.bias is not None:
    ...
    def deviation_loss(self) -> torch.Tensor:
        for i, (_weight, _weight_origin) in enumerate(zip(self.weight, self.weight_origin)):
            if i:
...
class MLP(nn.Module):
    def __init__(
            self, i_dim: typing.Tuple[int], o_dim: typing.Tuple[int], dropout: typing.Tuple[float],
            bias: bool = True, batch_normalization: bool = False, activation: bool = True
    ) -> None:
        super().__init__()
        self.i_dim = i_dim
        self.o_dim = o_dim
