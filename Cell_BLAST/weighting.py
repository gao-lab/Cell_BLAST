...
from . import utils, config, data

_identity = lambda x, y: 1 if x == y else 0

def calc_weights(adata: ad.AnnData,
    ...
    for _add_weight, _batch_effect in zip(add_weight, batch_effect):
        ...
        if _add_weight:
            ...
            if config.SUPERVISION is not None:
                ...
                adata.uns[config._WEIGHT_PREFIX_ + _batch_effect + '_sum_cluster'] = sum_cluster
                adata.uns[config._WEIGHT_PREFIX_ + _batch_effect + '_num_clusters'] = num_clusters
                ...
                similarity, raw_similarity = \
                    get_unsupervised_similarity(similarity_latent[mask], num_batch, cluster, sum_cluster, num_clusters, volume)
                adata.uns[config._WEIGHT_PREFIX_ + _batch_effect + '_raw_similarity'] = raw_similarity


            weight = get_weight(batch, num_batch, cluster, sum_cluster, num_clusters, volume, similarity)
...
def get_supervised_cluster(all_labels: typing.List[str],
                            batch: np.ndarray,
                            num_batch: int) -> typing.Tuple[np.ndarray, int, typing.List[int], np.ndarray]:
...
def get_unsupervised_cluster(clustering_latent: np.ndarray,
                            batch: np.ndarray,
                            num_batch: int,
                            random_seed: int) -> typing.Tuple[np.ndarray, int, typing.List[int], np.ndarray]:
    ...
    for i in range(num_batch):
        if config.NO_CLUSTER:
            ...
            sc.tl.leiden(curr_latent, random_state = random_seed, resolution = config.RESOLUTION)
...
def get_unsupervised_similarity(similarity_latent: np.ndarray,
                            ...
                            sum_cluster: int,
                            num_clusters: typing.List[int],
                            volume: np.ndarray) -> typing.Tuple[np.ndarray]:
...
def get_weight(batch: np.ndarray,
                num_batch: int,
                cluster: np.ndarray,
                sum_cluster: int,
                num_clusters: typing.List[int],
...
def plot_clustering_confidence(adata: ad.AnnData,
                                batch_effect: str,
    ...
    adata.uns[config._WEIGHT_PREFIX_ + batch_effect + '_main_cell_types'] = main_cell_types
...
def plot_similarity_confidence(adata: ad.AnnData,
                                batch_effect: str,
                                similarity: typing.Callable = _identity) -> None:
    ...
    main_cell_types = adata.uns[config._WEIGHT_PREFIX_ + batch_effect + '_main_cell_types']
    raw_similarity = adata.uns[config._WEIGHT_PREFIX_ + batch_effect + '_raw_similarity']
    ...
    print("true positive = %d" % ((truth == 1) & (raw_similarity == 1) & (mask == 1)).sum())
    print("true negative = %d" % ((truth == 0) & (raw_similarity == 0) & (mask == 1)).sum())
    print("false positive = %d" % ((truth == 0) & (raw_similarity == 1) & (mask == 1)).sum())
    print("false negative = %d" % ((truth == 1) & (raw_similarity == 0) & (mask == 1)).sum())
    plt.figure(figsize=(7,7))
...
def plot_strategy(adata: ad.AnnData,
                    batch_effect: str,
                    target: str,
...
def get_default_similarity_space(adata: ad.AnnData,
                                genes: typing.List[str],
                                batch_effect: str,
                                random_seed: int) -> np.ndarray:
...
def get_MNN(num_batch: int,
    ...
    for i in range(num_batch):
        for j in range(num_batch):
            xy_dist = torch.tensor(similarity[num_clusters[i]:num_clusters[i+1], num_clusters[j]:num_clusters[j+1]])
            ...
            similarity[num_clusters[i]:num_clusters[i+1], num_clusters[j]:num_clusters[j+1]] = mnn_idx.float().numpy()
