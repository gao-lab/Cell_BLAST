...
def normalize(adata: ad.AnnData, target: float = 10000.0) -> None:
    ...
    adata.X = adata.X.multiply(normalizer).tocsr() \
        if scipy.sparse.issparse(adata.X) \
...
def find_variable_genes(
    ...
    var = np.asarray(np.mean(
        X.power(2) if scipy.sparse.issparse(X) else np.square(X),
        axis=0
...
def _expanded_subset(
        mat: typing.Union[scipy.sparse.spmatrix, np.ndarray], idx: np.ndarray,
        axis: int = 0, fill: typing.Any = 0
) -> typing.Union[scipy.sparse.spmatrix, np.ndarray]:
    assert axis in (0, 1)
    expand_size = max(idx.max() - mat.shape[axis] + 1, 0)
    if axis == 0:
        if scipy.sparse.issparse(mat):
            expand_mat = scipy.sparse.lil_matrix((expand_size, mat.shape[1]), dtype=mat.dtype)
            if fill != 0:
                expand_mat[:] = fill
        ...
        if scipy.sparse.issparse(mat):
            expand_mat = scipy.sparse.lil_matrix((mat.shape[0], expand_size), dtype=mat.dtype)
            if fill != 0:
                expand_mat[:] = fill
...
def select_vars(adata: ad.AnnData, var_names: typing.List[str]) -> ad.AnnData:
    ...
    if new_var_names.size > 0:  # pragma: no cover
        utils.logger.warning(
            ...
            len(new_var_names), len(var_names)
        )
...
def map_vars(
        adata: ad.AnnData, mapping: pd.DataFrame,
...
def annotation_confidence(
        adata: ad.AnnData, annotation: typing.Union[str, typing.List[str]],
...
def write_table(adata: ad.AnnData, filename: str, orientation: str = "cg", **kwargs) -> None:
    r"""
    ...
    if orientation == "cg":
        df = pd.DataFrame(
            utils.densify(adata.X),
            index=adata.obs_names,
            columns=adata.var_names
        )
    elif orientation == "gc":
        df = pd.DataFrame(
            utils.densify(adata.X.T),
            index=adata.var_names,
...
def read_table(
        filename: str, orientation: str = "cg",
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, (slice, np.ndarray)):
            return OrderedDict([
                ...
                (item, self.data_dict[item][idx]) for item in self.data_dict
            ])
